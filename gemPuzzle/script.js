...
document.querySelector("body").innerHTML =
  '<div class="container"> <div class = "buttonPlace"></div> <div class = "movesAndTime"></div> <div class = "game-container"></div> </div>';

document.querySelector(".buttonPlace").innerHTML =
  '<button class="shuffle simpleButton">Shuffle and start</button> <button class = "stop grayButton">Stop</button> <button class="save simpleButton">Save</button> <button class="results simpleButton">Results</button>';

document.querySelector(".game-container").innerHTML =
  '<canvas id="game" width="400" height="400"></canvas>';

document.querySelector(".movesAndTime").innerHTML =
  '<div id = "moves"><h1>Moves: </h1> <h1>0</h1></div> <div id = "time"><h1>Time: </h1> <h1>00:00:00</h1></div>';
...
cvs.addEventListener("click", (e) => {
  ...
  if (stillOnHovered(clientX, clientY)) {
    ...
    if (emptyCell) {
      ...
      moves++;
      movesBlock.innerHTML = `<h1>Moves: ${moves}<h1>`;
...
async function drawBoard() {
  ...
  setInterval(() => {
    timeBlock.innerHTML = `<h1>Time: ${formatSeconds(++time)}</h1>`;
  }, 1000);
}
...
alert(
  "Прошу проверить мою работу ближе к концу кросс-чека, не успел выполнить некоторые моменты. За ранее спасибо "
);
